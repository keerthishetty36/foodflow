...
router.get("/users", authenticate, requirePermission("users.view"), asyncHandler(async (_req, res) => res.json({ data: await prisma.user.findMany({ select: { id: true, name: true, email: true, role: true, active: true, createdAt: true, customRole: true } }) }))); router.post("/users", authenticate, requirePermission("users.create"), asyncHandler(async (req: AuthRequest, res) => { const body = z.object({ name: z.string().min(2), email: z.string().email(), password: z.string().min(8), roleId: z.string().optional(), role: z.nativeEnum(RoleEnum).optional().default(RoleEnum.CASHIER) }).parse(req.body); const user = await prisma.user.create({ data: { ...body, passwordHash: await hashPassword(body.password) } }); await audit(req, "CREATE", "User", user.id); res.status(201).json({ data: { id: user.id, name: user.name, email: user.email, role: user.role } }) })); router.patch("/users/:id", authenticate, requirePermission("users.edit"), asyncHandler(async (req, res) => { const body = z.object({ name: z.string().min(2).optional(), active: z.boolean().optional(), password: z.string().min(8).optional(), roleId: z.string().nullable().optional(), role: z.nativeEnum(RoleEnum).optional() }).parse(req.body); const data: any = { ...body }; if (body.password) { data.passwordHash = await hashPassword(body.password); delete data.password } res.json({ data: await prisma.user.update({ where: { id: String(req.params.id) }, data, select: { id: true, name: true, email: true, role: true, active: true } }) }) }));
