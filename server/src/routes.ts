...
router.delete("/categories/:id", requireAuth, admin, asyncHandler(async (req, res, next) => {
...
const normalizeMenuInput = (body: unknown) => { const input = body && typeof body === "object" ? body as Record<string, unknown> : {}; const normalized = Object.fromEntries(Object.entries(input).map(([key, value]) => [key, typeof value === "string" ? value.trim() : value])); for (const field of ["barcode", "sku"]) { if (normalized[field] === "") normalized[field] = null } return { ...normalized, price: normalized.price === undefined ? undefined : Number(normalized.price), costPrice: normalized.costPrice === undefined ? undefined : Number(normalized.costPrice), vegType: normalizeVegType(normalized.vegType) }; };
const menuSchema = z.object({ name: z.string().trim().min(2), description: z.string().optional(), image: z.string().url().optional(), categoryId: z.string().trim().min(1), price: z.number().finite().positive(), costPrice: z.number().finite().min(0), tax: z.coerce.number().min(0).max(100).optional(), discount: z.coerce.number().min(0).max(100).optional(), preparationTime: z.coerce.number().int().min(0).optional(), available: z.boolean().optional(), vegType: z.enum(["VEG", "NON_VEG", "EGG"]), barcode: z.string().min(1).nullable().optional(), sku: z.string().min(1).nullable().optional(), calories: z.coerce.number().int().positive().optional() });
