...
router.get("/auth/me", authenticate, asyncHandler(async (req: AuthRequest, res) => { const user = await prisma.user.findUnique({ where: { id: req.user!.sub }, include: { customRole: true } }); res.json({ data: { user: { id: user?.id, name: user?.name, email: user?.email, role: user?.role, active: user?.active, permissions: user?.customRole?.permissions || (user?.role === RoleEnum.ADMIN ? ["*"] : ["pos.view", "pos.bill", "orders.view", "orders.update", "tables.view", "menu.read"]) }, role: user?.role } }) }));
