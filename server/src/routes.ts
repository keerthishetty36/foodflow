...
router.get("/orders/:id/receipt",requireAuth,staff,asyncHandler(async(req,res)=>{const order=await prisma.order.findUniqueOrThrow({where:{id:String(req.params.id)},include:{items:true,table:true,customer:true,payments:true,cashier:{select:{name:true}}}});writeReceipt(res,order)}));

router.get("/reports/sales",requireAuth,admin,asyncHandler(async(req,res)=>{const from=req.query.from?new Date(String(req.query.from)):new Date(Date.now()-30*864e5);const payments=await prisma.payment.findMany({where:{status:PaymentStatus.PAID,paidAt:{gte:from}},include:{order:{include:{items:true}}}}),orders=[...new Map(payments.map(payment=>[payment.orderId,payment.order])).values()];res.json({data:{sales:payments.reduce((sum,payment)=>sum+payment.amount,0),orders:orders.length,tax:orders.reduce((sum,order)=>sum+order.taxTotal,0),profit:orders.reduce((sum,order)=>sum+order.items.reduce((itemSum,item)=>itemSum+(item.unitPrice-item.costPrice)*item.quantity,0),0),rows:orders}})}));
