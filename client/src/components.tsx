import type { ReactNode } from "react";import { NavLink } from "react-router-dom";import { LayoutDashboard,UtensilsCrossed,ReceiptText,ChefHat,Package,Settings,LogOut,Table2,BarChart3,ShoppingCart,ClipboardList,Bell } from "lucide-react";import { useAuthStore } from "./store";import { api } from "./api";
const ADMIN_ROLE="ADMIN" as const;
const links=[['/dashboard','Dashboard',LayoutDashboard],['/pos','POS Billing',ShoppingCart],['/kitchen','Kitchen Screen',ChefHat],['/orders','Orders',ReceiptText],['/menu','Menu',UtensilsCrossed],['/tables','Tables',Table2],['/inventory','Inventory',Package],['/suppliers','Suppliers',ClipboardList],['/reports','Reports',BarChart3],['/notifications','Notifications',Bell],['/settings','Settings',Settings],['/cashier','Cashier Dashboard',ShoppingCart]] as const;
export function Shell({children}:{children:ReactNode}){const {user,clear}=useAuthStore();const visible=links.filter(([path])=>user?.role===ADMIN_ROLE||['/pos','/kitchen','/orders','/tables'].includes(path));return <div className="min-h-screen lg:flex"><aside className="w-full border-b bg-white p-4 dark:border-slate-800 dark:bg-slate-900 lg:fixed lg:inset-y-0 lg:w-64 lg:border-r lg:border-b-0"><div className="mb-6 flex items-center gap-2 text-xl font-bold text-brand-600"><UtensilsCrossed/> FoodFlow</div><nav className="grid grid-cols-3 gap-1 lg:block">{visible.map(([to,label,Icon])=><NavLink key={to} to={to} end={to==='/dashboard'} className="nav-link"><Icon size={18}/><span className="hidden lg:inline">{label}</span></NavLink>)}</nav><button className="nav-link mt-4 w-full" onClick={async()=>{await api.post('/auth/logout').catch(()=>{});clear()}}><LogOut size={18}/><span className="hidden lg:inline">Sign out</span></button></aside><main className="min-w-0 flex-1 p-4 lg:ml-64 lg:p-8"><header className="mb-6 flex items-center justify-between"><div><p className="text-sm text-slate-500">{new Date().toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'})}</p><h1 className="text-xl font-bold">Welcome, {user?.name}</h1></div><span className="rounded-full bg-brand-50 px-3 py-1 text-xs font-bold text-brand-700">{user?.role}</span></header>{children}</main></div>}
